package uk.easys.easymarketing.woo.data.auth
...
class AuthIntercepter(private val consumerKey: String, private val consumerSecret: String) :
    Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        for (entry in params) builder.addQueryParameter(entry.name, entry.value)
    ...
    fun getOauthParams(chain: Interceptor.Chain): ArrayList<NameValuePair> {
        ...
        oauth_signature =
            getOauth_signature(chain.request().method(), request_url, consumerSecret, encodedParams)
    ...
    fun getOauth_nonce() = StringBuilder((Math.random() * 100000000.0).toString()).toString()
    ...
    fun getOauth_signature(
        ...
        try {
            ...
            signature =
                Base64.encodeToString(mac.doFinal(string_to_sign.toByteArray(charset("utf-8"))), 0)
                    .trim { it <= ' ' }
